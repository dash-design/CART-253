...
function generateMaze() {
    ...
    let startRow = Math.floor(Math.random() * ((rows - 1) / 2)) * 2 + 1;
    let startCol = Math.floor(Math.random() * ((cols - 1) / 2)) * 2 + 1;
...
function removeWall(r, c) {
    ...
    for (let [sR, sC] of sides) {
        let nR = r + sR;
        let nC = c + sC;
        if (checkRemoval(nR, nC)) {
            maze[r + sR / 2][c + sC / 2] = " ";
            maze[nR][nC] = " ";
            remove(nR, nC);
        }
